...
{ 
    ...
    {
        private readonly static log4net.ILog LOG = log4net.LogManager.GetLogger("AirportController");
        private IAirportService _airportService;
        private IAccountUserService _accountUserService;
        private ICityLocationService _cityLocationService;
        public AirportController()
        {
            _airportService = new AirportService(new AirportRepository(), new AirportValidation());
        ...
        public dynamic GetList(string _search, long nd, int rows, int? page, string sidx, string sord, string filters = "")
        {
            ...
            // Get Data
            var q = _airportService.GetQueryable().Where(x => x.OfficeId == officeid);
        ...
        public dynamic GetLookUp(string _search, long nd, int rows, int? page, string sidx, string sord, string filters = "")
        {
            ...
            // Get Data
            var q = _airportService.GetQueryable().Where(x => x.OfficeId == officeid && x.IsDeleted == false);
        ...
        public dynamic GetInfo(int Id)
        {
            ...
            try
            {
                model = _airportService.GetObjectById(Id);
        ...
        [HttpPost]
        public dynamic Insert(Airport model)
        {
            try
            {
                ...
                model = _airportService.CreateObject(model,_cityLocationService);
        ...
        [HttpPost]
        public dynamic Update(Airport model)
        {
            try
            {
                ...
                model.OfficeId = _accountUserService.GetObjectById(userId).OfficeId;
                var data = _airportService.GetObjectById(model.Id);
                data.Abbrevation = model.Abbrevation;
                data.Name = model.Name;
                model = _airportService.UpdateObject(data,_cityLocationService);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public dynamic Delete(Airport model)
        {
            try
            {
                ...
                model = _airportService.SoftDeleteObject(data);
            }
            catch (Exception ex)
            {
