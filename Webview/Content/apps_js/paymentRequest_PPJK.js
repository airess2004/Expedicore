$(document).ready(function () {
    ...
    function FirstLoad(InvoicesNo) {
        ...
        $.ajax({
            dataType: "json",
            url: base_url + "PaymentRequest/GetInfo?Id=" + InvoiceId + "&JobId=" + jobId,
            success: function (result) {
                if (JSON.stringify(result.Errors) != '{}') {
                    ...
                    if (result.JenisPaymentRequest == "G")
                    ...
                    // Invoice Detail
                    if (result.ListPaymentRequestDetail != null) {
                        if (result.ListPaymentRequestDetail.length > 0) {
                            for (var i = 0; i < result.ListPaymentRequestDetail.length; i++) {
                                addInvDetail(0,
                                    result.ListPaymentRequestDetail[i].Id,
                                    result.ListPaymentRequestDetail[i].EPLDetailId,
                                    result.ListPaymentRequestDetail[i].AccountCode,
                                    result.ListPaymentRequestDetail[i].AccountName,
                                    result.ListPaymentRequestDetail[i].Description,
                                    result.ListPaymentRequestDetail[i].Type,
                                    result.ListPaymentRequestDetail[i].AmountCrr,
                                    result.ListPaymentRequestDetail[i].Amount,
                                    result.ListPaymentRequestDetail[i].Quantity,
                                    result.ListPaymentRequestDetail[i].PerQty,
                                    result.ListPaymentRequestDetail[i].CodingQuantity,
                                    result.ListPaymentRequestDetail[i].Sign,
                                    result.ListPaymentRequestDetail[i].HFPSinfo,
                                    result.ListPaymentRequestDetail[i].PercentVat,
                                    result.ListPaymentRequestDetail[i].VatId,
                                    result.ListPaymentRequestDetail[i].Dmr,
                                    result.ListPaymentRequestDetail[i].DmrContainerDetail,
                                    result.ListPaymentRequestDetail[i].DmrContainer);
                            }
                        }
                    }
    ...
    $('#btnSEcustomer').click(function () {
        ...
        var lookUpURL = base_url + 'MstContact/GetLookUpEPLCost';
    ...
    $('#btnInvaccount').click(function () {
        ...
        // Invoice
        var accountUrl = 'EstimateProfitLoss/GetLookUpCost';
        if ($('#optSEpaymentfromgpr').is(':checked')) {
            // General Invoice
    ...
    jQuery("#lookup_table_account").jqGrid({
        ...
        colModel: [{ name: 'code', index: 'MasterCode', width: 80, align: "center" },
                  { name: 'name', index: 'Contact', width: 200 },
                  { name: 'desc', index: 'Description', width: 200, hidden: true },
    ...
    $('input[name=optSEDC], input[name=optSEpaymentfrom]').click(function () {
        ...
        $('#txtSEpaymentusd, #txtSEpaymentidr').numberbox('setValue', 0);
    ...
    $('#lookup_btn_add_invoice_detail').click(function () {
        ...
        var submitURL = (InvoiceId != undefined && InvoiceId != "") ? (vInvoiceDetailEdit == 0) ? base_url + "PaymentRequest/InsertDetail" : base_url + "PaymentRequest/UpdateDetail" : "";
        var isValid = true;
        var message = "OK";
        var amountvat = 0;
        if ($('#ddlSEvat').val() != '') {
        ...
        SaveInvoiceDetail(submitURL, invoiceDetailId, eplDetailId, $('#txtSEkdcustomer').data('kode'), $('#txtInvkdaccount').data("kode"), $('#txtInvnmaccount').val(),
                        ...
                        $('#txtInvkdaccount').data("codingquantity"), sign, currencyType, amount, $('#ddlSEvat option:selected').text(), amountvat, vatId, function (result) {
                            ...
                            {
                                addInvDetail(vInvoiceDetailEdit, result.Id, eplDetailId, $('#txtInvkdaccount').data("kode"), $('#txtInvnmaccount').val(), $('#txtInvdesc').val(),
   ...
   function SaveInvoiceDetail(submitURL, v_invoicedetailId, v_epldetailId, v_customerId, v_accountId, v_accountName, v_description,
        ...
        jQuery("#table_invoice_detail").jqGrid({
            ...
            colModel: [
                ...
                { name: 'vat', index: 'vat', width: 55, align: "right", formatter: 'currency', hidden: true, formatoptions: { decimalSeparator: ".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", suffix: "%", defaultValue: '0' } },
        ...
        $('#seaimport_form_btn_save').click(function () {
            ...
            var submitURL = base_url + "PaymentRequest/Insert";
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    InvoicesNo: $("#InvoicesNo").val(), JenisPaymentRequest: invoiceType, ETD: $('#txtSEetd').val(), CurrencyId: amountCrr, DebetCredit: debetCredit,
                }),
                success: function (result) {
                    if (JSON.stringify(result.Errors) != '{}') {
                        for (var key in result.Errors) {
                        ...
                        window.location = base_url + "PaymentRequest/Detail?Id=" + result.paymentRequestId + "&JobId=" + jobId;
                    }
                }
            });
        });
        ...
        $("#btnDialogPrintFixedDraftOk").click(function () {
            ...
            window.open(base_url + "PaymentRequest/Prints?Id=" + InvoiceId + "&bo=" + bo + "&fd=" + fd);
        });
        ...
        $("#btnDialogPrintFixedDraftNo").click(function () {
            ...
            window.open(base_url + "PaymentRequest/Print?Id=" + InvoiceId + "&bo=" + bo + "&fd=" + fd);
        });
        // END Print
                        ...
                        success: function (result) {
                            if (result.isValid) {
                                $.messager.alert('Information', result.message, 'info', function () {
                                    window.location = base_url + "PaymentRequest/Detail?Id=" + InvoiceId + "&JobId=" + jobId;
                                });
                            }
                            else {
        ...
        $("#invoice_btn_add_new").click(function () {
            ...
            $.messager.confirm('Confirm', 'Are you sure you want to create NEW Invoice?', function (r) {
                if (r) {
                    ...
                    $.ajax({
                        dataType: "json",
                        url: base_url + "PaymentRequest/AllowNewInvoice",
                        success: function (result) {
                            if (!result.isValid) {
        ...
        $('#nav_next').click(function () {
            $.ajax({
                url: base_url + "PaymentRequest/NextInvoice?CurrentId=" + InvoiceId + "&JobId=" + jobId,
                success: function (result) {
                    if (result.isValid) {
                        window.location = base_url + "PaymentRequest/detail?Id=" + result.InvoiceId + "&JobId=" + jobId;
                    }
                    else {
                        $.messager.alert('Warning', result.message, 'warning');
                    }
        ...
        $('#nav_prev').click(function () {
            $.ajax({
                url: base_url + "PaymentRequest/PrevInvoice?CurrentId=" + InvoiceId + "&JobId=" + jobId,
                success: function (result) {
                    if (result.isValid) {
                        window.location = base_url + "PaymentRequest/detail?Id=" + result.InvoiceId + "&JobId=" + jobId;
                    }
        ...
        $('#nav_first').click(function () {
            $.ajax({
                url: base_url + "PaymentRequest/FirstInvoice?CurrentId=" + InvoiceId + "&JobId=" + jobId,
                success: function (result) {
                    if (result.isValid) {
                        window.location = base_url + "PaymentRequest/detail?Id=" + result.InvoiceId + "&JobId=" + jobId;
                    }
        ...
        $('#nav_last').click(function () {
            $.ajax({
                url: base_url + "PaymentRequest/LastInvoice?CurrentId=" + InvoiceId + "&JobId=" + jobId,
                success: function (result) {
                    if (result.isValid) {
                        window.location = base_url + "PaymentRequest/detail?Id=" + result.InvoiceId + "&JobId=" + jobId;
                    }
