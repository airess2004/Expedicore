...
$(document).ready(function () {
    ...
    function ReloadGrid() {
        $("#list").setGridParam({ url: base_url + 'Vat/GetList' }).trigger("reloadGrid");
    }

    function ClearData() {
        $('#Abbrevation').val('').text('').removeClass('errormessage');
    ...
    //GRID +++++++++++++++
    $("#list").jqGrid({
        url: base_url + 'Vat/GetList',
        datatype: "json",
        colNames: ['ID', 'Name', 'Value','Created At', 'Updated At'],
        colModel: [
    			  { name: 'id', index: 'MasterCode', width: 80, align: "center" },
    			  { name: 'Name', index: 'Name', width: 130, align: "center" },
      	          { name: 'Value', index: 'Value', width: 100, align: "right", formatter: 'currency', formatoptions: { decimalSeparator: ".", thousandsSeparator: ",", decimalPlaces: 2, prefix: "", suffix: "", defaultValue: '0.00' } },
    ...
    $('#btn_edit').click(function () {
        ...
        if (id) {
            ...
            $.ajax({
                dataType: "json",
                url: base_url + "Vat/GetInfo?Id=" + id,
    ...
    $("#form_btn_save").click(function () {
        ...
        if (id != undefined && id != '' && !isNaN(id) && id > 0) {
            submitURL = base_url + 'Vat/Update';
        }
            // Insert
        else {
            submitURL = base_url + 'Vat/Insert';
        }
        ...
        $.ajax({
            ...
            url: submitURL,
            data: JSON.stringify({
                Id: id, Name: $("#Name").val(), Value: $("#Value").numberbox('getValue')
            }),
            async: false,
            cache: false,
            timeout: 30000,
