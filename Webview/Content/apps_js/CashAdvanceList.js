$(document).ready(function () {
    ...
    function ReloadGrid() {
        ...
        $("#list_paymentvoucher").setGridParam({ url: base_url + 'CashAdvance/GetList', postData: { filters: null, JobId: value }, page: 'last' }).trigger("reloadGrid");
    }

    //GRID LOOKUP +++++++++++++++
    $("#list_paymentvoucher").jqGrid({
        url: base_url + 'CashAdvance/GetList',
        datatype: "json",
        colNames: ['Del', 'Confirmed', 'C.A No', 'Paid', 'Employee Name', 'Cash IDR', 'Cash USD', 'Created At', 'Created By', 'Updated At'],
        colModel: [
			      ...
			      { name: 'isConfirmed', index: 'Paid', width: 60, align: "center", sortable: false, stype: 'select', editoptions: { value: ":All;true:Yes;false:No" }, formatter: 'select' },
                  { name: 'reference', index: 'EmployeeName', width: 180 },
				  { name: 'totalpayment', index: 'CashAdvanceIDR', width: 120, align: "right", formatter: 'currency', formatoptions: { decimalSeparator: ".", thousandsSeparator: ",", decimalPlaces: 2, prefix: "", suffix: "", defaultValue: '0.00' } },
				  { name: 'totalpayment', index: 'CashAdvanceUSD', width: 120, align: "right", formatter: 'currency', formatoptions: { decimalSeparator: ".", thousandsSeparator: ",", decimalPlaces: 2, prefix: "", suffix: "", defaultValue: '0.00' } },
    ...
    $("#paymentvoucher_btn_edit, #paymentvoucher_btn_del").click(function () {
        ...
        if (pvid) {
            ...
            if (buttonID == "paymentvoucher_btn_edit") {
                window.location = base_url + "CashAdvance/Detail?Id=" + pvid + "&JobId=" + value;
            }
                // On Delete
            else {
                DeleteCashAdvance(pvid);
            }
    ...
    $("#paymentvoucher_btn_print").click(function () {
        ...
        if (pvId) {
            var ret = jQuery("#list_paymentvoucher").jqGrid('getRowData', pvId);
            PrintCashAdvance(pvId);
        } else {
            $.messager.alert('Information', 'Please Select Data...!!', 'info');
        }
    });

    function DeleteCashAdvance(pvId) {
        $.messager.confirm('Confirm', 'Are you sure you want to delete the selected record?', function (r) {
            if (r) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    url: base_url + "CashAdvance/Delete",
                    data: JSON.stringify({
                        Id: pvId
                    }),
                    success: function (result) {
    ...
    function PrintCashAdvance(pvId) {
        window.open(base_url + "CashAdvance/Print?Id=" + pvId);
    }
    ...
    $('#btn_confirm').click(function () {
        ...
        if (id) {
            ...
            $.messager.confirm('Confirm', 'Are you sure you want to confirm record?', function (r) {
                if (r) {
                    $.ajax({
                        url: base_url + "CashAdvance/Confirm",
                        type: "POST",
                        contentType: "application/json",
                        data: JSON.stringify({
                            Id: id,
    ...
    $('#btn_unconfirm').click(function () {
        ...
        if (id) {
            ...
            $.messager.confirm('Confirm', 'Are you sure you want to unconfirm record?', function (r) {
                if (r) {
                    $.ajax({
                        url: base_url + "CashAdvance/Unconfirm",
                        type: "POST",
                        contentType: "application/json",
                        data: JSON.stringify({
                            Id: id,
