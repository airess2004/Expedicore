$(document).ready(function () {
    ...
    function FirstLoad() {
        ...
        $.ajax({
            dataType: "json",
            url: base_url + "paymentvoucher/GetInfo?Id=" + PVId,
            success: function (result) {
                if (JSON.stringify(result.Errors) != '{}') {
                    ...
                    if (result.ListPaymentVoucherDetail != null) {
                        if (result.ListPaymentVoucherDetail.length > 0) {
                            for (var i = 0; i < result.ListPaymentVoucherDetail.length; i++) {
                                AddPaymentDetail(0, result.ListPaymentVoucherDetail[i].Id,
                                    result.ListPaymentVoucherDetail[i].AccountId,
                                    result.ListPaymentVoucherDetail[i].AccountCode,
                                    result.ListPaymentVoucherDetail[i].AccountName,
                                    result.ListPaymentVoucherDetail[i].Description,
                                    result.ListPaymentVoucherDetail[i].DCNote,
                                    result.ListPaymentVoucherDetail[i].AmountUSD,
                                    result.ListPaymentVoucherDetail[i].AmountIDR,
                                    result.ListPaymentVoucherDetail[i].PayableCode,
                                    result.ListPaymentVoucherDetail[i].ShipmentOrderNo,
                                    result.ListPaymentVoucherDetail[i].RefRate,
                                    result.ListPaymentVoucherDetail[i].ExRateId,
                                    result.ListPaymentVoucherDetail[i].ShipmentOrderId,
                                    result.ListPaymentVoucherDetail[i].RemainingAmountUSD,
                                    result.ListPaymentVoucherDetail[i].RemainingAmountIDR,
                                   result.ListPaymentVoucherDetail[i].PaymentCash,
                                   result.ListPaymentVoucherDetail[i].PayableId,
                                   result.ListPaymentVoucherDetail[i].RefId,
                                   result.ListPaymentVoucherDetail[i].PayableId,
                                   result.ListPaymentVoucherDetail[i].PVBank);
                            }
                        }
                    }
                    CalculatePayment();
    ...
    $('#btnPVDrefno').click(function () {
        ...
        var listExistedInvoice = [];
            ...
            pvd_lookupUrl_settlement_for = 'PaymentRequest/GetLookUpPV';
    ...
    function ValidateViewPaymentVoucherDetail() {
        var isValid = true;
    ...
    $('#btn_add_payment_voucher_detail').click(function () {
        ...
        var isValid = ValidateViewPaymentVoucherDetail();
        if (isValid) {
    ...
    $('#lookup_btn_add_payment_voucher_detail').click(function () {
        ...
        if (!$('#rbSettlementForGeneral').is(':checked') && $.trim($('#txtPVDrefno').val()) == "") {
            ...
            var submitURL = (PVId != undefined && PVId != "") ? (pvDetailEdit == 0) ? base_url + "PaymentVoucher/InsertDetail" : base_url + "PaymentVoucher/UpdateDetail" : "";
            var isValid = true;
            var message = "OK";
    ...
    function SavePVDetail(submitURL, v_pvDetailId, v_shipmentNo, v_accountId, v_desc, v_dcnote, v_amountUsd, v_amountIdr,
        ...
        if (submitURL != undefined && submitURL != "") {
            $.ajax({
                ...
                url: submitURL,
                data: JSON.stringify({
                    PaymentVoucherId: PVId, Id: v_pvDetailId,
    ...
    $('#btn_save_cash').click(function () {
        ...
        if (id) {
            ...
            var submitURL = (PVId != undefined && PVId != "") ? base_url + "PaymentVoucher/UpdatePVDetail" : "";
            var isValid = true;
            var message = "OK";
    ...
    // Payment Voucher Detail
    function PopulatePaymentVoucherDetail() {
        var pvDetail = [];
        var data = $("#table_payment_voucher_detail").jqGrid('getGridParam', 'data');
        for (var i = 0; i < data.length; i++) {
            var obj = {};
    ...
    $('#lookup_btn_add_payment_voucher_detail_bank').click(function () {
        ...
        var submitURL = (PVId != undefined && PVId != "") ? (pvBankEdit == 0) ? base_url + "PaymentVoucher/InsertPVBank" : base_url + "PaymentVoucher/UpdatePVBank" : "";
        var isValid = true;
        var message = "OK";
            ...
            if (r) {
                ...
                if (PVId != undefined && PVId != "") {
                    $.ajax({
                        contentType: "application/json",
                        type: 'POST',
                        url: base_url + 'PaymentVoucher/DeletePVBank',
                        data: JSON.stringify({
                            Id: v_pvBankId
                        }),
                        async: false,
    ...
    function SaveAll() {
        ...
        var submitURL = base_url + "PaymentVoucher/Insert";
        ...
        $.ajax({
            ...
            success: function (result) {
                if (JSON.stringify(result.Errors) != '{}') {
                    ...
                    window.location = base_url + "PaymentVoucher/Detail?Id=" + result.paymentVoucherId;
                }
            }
        });
    }
    ...
    function PopulatePaymentVoucherDetailBank() {
        var pvDetailBank = [];
    ...
    $('#paymentvoucher_form_btn_print').click(function () {
        ...
        //$.ajax({
        //    dataType: "json",
        //    url: base_url + "paymentvoucher/GetPaymentVoucherInfo?Id=" + PVId,
        //    success: function (result) {
        //        if (result.isValid) {
    ...
    $('#btnDialogPrintYes').click(function () {
        ...
        $('#dialogPrint').dialog('close');
        window.open(base_url + "PaymentVoucher/Print?Id=" + PVId + "&PrintedOn=" + dateprint);
    });
    ...
    $('#paymentvoucher_btn_add_new').click(function () {
        $.messager.confirm('Confirm', 'Are you sure you want to create New Payment Voucher?', function (r) {
            if (r) {
                //$.ajax({
                //    dataType: "json",
                //    url: base_url + "PaymentVoucher/IsAllowNew",
                //    success: function (result) {
                //        if (!result.isValid) {
                //            $.messager.alert('Information', result.message, 'info');
                //        }
                //        else {
                            window.location = base_url + "PaymentVoucher/Detail";
                //        }
                //    }
                //});
            }
