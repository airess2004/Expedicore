...
{ 
    public class ReceiptVoucherDetailValidator : IReceiptVoucherDetailValidator
    {
        ...
        {
            if (receiptVoucherDetail.AmountUSD < 0 || receiptVoucherDetail.AmountIDR < 0)
            {
        ...
        {
            Receivable receivable = _receivableService.GetObjectById(receiptVoucherDetail.ReceivableId);
            if ((receiptVoucherDetail.AmountIDR + receiptVoucherDetail.AmountUSD) > receivable.Amount)
            {
                                                                                 ...
                                                                                 IReceiptVoucherDetailService _receiptVoucherDetailService)
        {
            ...
            foreach (var detail in receiptVoucherDetails)
            {
                TotalReceiptVoucherDetails += detail.AmountIDR + detail.AmountUSD;
            }
            ReceiptVoucher receiptVoucher = _receiptVoucherService.GetObjectById(receiptVoucherDetail.ReceiptVoucherId);
            if (receiptVoucher.TotalAmountIDR < TotalReceiptVoucherDetails)
            {
                decimal sisa = receiptVoucher.TotalAmountIDR - TotalReceiptVoucherDetails + receiptVoucherDetail.AmountIDR;
